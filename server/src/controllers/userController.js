...
const { setupProfile, sendLoginCode, verifyLoginCode, sendEmailCode, verifyEmailCode, changeProfile, getCodeInfo, getProfile, setProfilePicture, deleteExistingProfilePicture, saveFile, getFiles, deleteFile, me } = require('../managers/userManager');
...
router.post(PATHS.files, mustBeAuth, MustBeSetup, async (req, res) => {
    try {
        const id = req.user._id;
        const files = await saveFile(id, req, res);
...
router.get(PATHS.files, mustBeAuth, MustBeSetup, async (req, res) => {
    try {
        const id = req.user._id;
...
router.delete(PATHS.file, mustBeAuth, MustBeSetup, async (req, res) => {
    try {
        const userID = req.user._id;
        const fileID = req.params?.fileID
        const files = await deleteFile(userID, fileID, req);
        res.status(200).json(files);
    } catch (err) {
        const error = formatErrorMessage(err);
        res.status(400).send({ message: error });
